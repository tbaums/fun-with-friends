...
printf '%s\n' "$PROFILE" > "$FWF_RUN/profile"   # persist the running profile (#530) so a respawn with no FWF_PROFILE cannot fall back to `example` and rebuild that profile's floor shape
