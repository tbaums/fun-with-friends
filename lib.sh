...
# QA-caught (#213 review): this used to close with "emitted only by a human
# keypress on the board, never by a role" -- true of neither path even
# before #213 (fwf-dash-act.sh's approve was always directly invocable by a
# role too; #213's own body admits this), and #213 made it materially worse
# by giving that same reachable path a first-class CLI verb "a name an
# agent already knows". That exact sentence is also baked verbatim into
# every role's own AUTHORIZATION GROUND RULES (lib.sh, the
# fwf_dev_prompt/fwf_pm_prompt/etc. renderer) as the JUSTIFICATION for
# trusting an AUTHORIZED verdict -- so an inaccurate claim here was not just
# a stale log line, it was propagating a false premise into every role's own
# reasoning. Fixed here AND in that ground-rules text (same edit, same
# reasoning) to the honest #213 §3 framing: provenance, not prevention. The
# OPERATIVE instruction (trust `fwf authz`'s verdict unconditionally, never
# reverse an AUTHORIZED issue) is unchanged either place -- only the
# no-longer-true justification for it.
fwf_ungate_comment_body() { # $1=issue-number $2=invocation description
  printf '**%s #%s** — %s; removing %s so implementers can claim it. This comment is the authorization signal of record (issue #150, anchored per issue #218) — a real, reviewable artifact recording who/how it was posted (issue #213; see fwf ungate --audit), not a technically human-only channel. No pane or input-box text is authorization; verify with: fwf authz %s' \
...
# --- role-prompt path validation (issue #467, defense-in-depth seam) --------
# A role must never ADOPT a role-prompt path outside its own profile (the
# #466 injection handed the conductor a test-fixture path under /tmp/…) — the
# PRIMARY defense is the liveness wrapper below (parking is caught from ANY
# cause), but this closes the narrower, cheaper case: reject the foreign path
# before it's ever treated as this role's own. $1=role $2=candidate-path ->
# on stdout, the resolved real path IFF it is exactly this role's own
# rendered prompt ($FWF_RUN/prompts/$PROFILE-$role.prompt); prints nothing
# and returns 1 for anything else, INCLUDING a missing/unreadable path
# (AC7: the unknown case takes the refuse branch, never a permissive one).
# `realpath -m` resolves symlinks and `..` traversal without requiring the
# target to exist, so a foreign path reached via either is compared on its
# REAL target (the edge case the ticket names), and a genuinely missing file
# still resolves to A path rather than erroring — "cannot confirm" and
# "confirmed foreign" must not collapse into the same silent success.
fwf_resolve_role_prompt_path() { # $1=role $2=candidate-path
  local role="$1" candidate="$2" resolved want_resolved
  [ -n "$role" ] && [ -n "$candidate" ] || return 1
  resolved="$(realpath -m -- "$candidate" 2>/dev/null || true)"
  [ -n "$resolved" ] || return 1
  want_resolved="$(realpath -m -- "$FWF_RUN/prompts/$PROFILE-$role.prompt" 2>/dev/null || true)"
  [ -n "$want_resolved" ] || return 1
  [ "$resolved" = "$want_resolved" ] || return 1
  printf '%s\n' "$resolved"
}

# --- parked-role escalation (issue #467) -------------------------------
# #467: a role that cannot proceed on some input (a foreign role-prompt path,
# or any other cause the loop/tick wrapper below observes as parking) must
# not go silent — the refusal has to land somewhere a human or the captain
# actually reads (AC3), deduped on (role, resolved input) with a STATED,
# persisted expiry so a re-injected foreign path recurring every ~22min (the
# observed #466 cadence) doesn't flood the channel, while a genuinely new
# occurrence after expiry still escalates (AC4). Persisted under
# $FWF_STATE_DIR, not in-memory, so it survives exactly the respawn that
# clears the parked pane (AC4's "respawn-durable" requirement). There is no
# issue/PR number for this class of event (a parked pane, not a ticket) —
# this uses the fallback the ticket itself names: "a durable state file plus
# a tick, never a blocking prompt." fwf-supervise.sh's existing routed
# per-pass output (the same channel WEDGED/LANE_STALE/WORKTREE_STALE already
# use) is what makes the record human-visible in practice.
FWF_PARKED_FLAG_EXPIRY_SECS="${FWF_PARKED_FLAG_EXPIRY_SECS:-1800}"

_fwf_parked_flag_key() { # $1=role $2=input -> stable key, no path chars
  printf '%s|%s' "$1" "$2" | cksum | awk '{print $1}'
}
_fwf_parked_flag_file() { # $1=role $2=input
  printf '%s/parked-flag/%s/%s' "$FWF_STATE_DIR" "$1" "$(_fwf_parked_flag_key "$1" "$2")"
}

# 0 = not yet flagged, or the prior flag has expired -> caller should
# escalate now. 1 = already flagged within FWF_PARKED_FLAG_EXPIRY_SECS ->
# dedupe, skip. A missing/malformed state file always allows escalation —
# same fail-open-on-absence shape as fwf_respawn_breaker_check.
fwf_parked_flag_check() { # $1=role $2=input
  local f flagged_at now
  f="$(_fwf_parked_flag_file "$1" "$2")"
  [ -f "$f" ] || return 0
  flagged_at="$(awk -F= '$1=="flagged_at"{print $2}' "$f" 2>/dev/null || true)"
  case "$flagged_at" in ''|*[!0-9]*) return 0;; esac
  now="$(date +%s)"
  [ $(( now - flagged_at )) -ge "$FWF_PARKED_FLAG_EXPIRY_SECS" ] && return 0
  return 1
}

# Persist the dedupe key (with a fresh timestamp) and append one line to the
# durable, human-readable escalation log. $1=role $2=input $3=reason
fwf_parked_flag_record() {
  local role="$1" input="$2" reason="$3" f log now
  f="$(_fwf_parked_flag_file "$role" "$input")"
  mkdir -p "$(dirname "$f")" 2>/dev/null
  now="$(date +%s)"
  printf 'flagged_at=%s\nrole=%s\ninput=%s\n' "$now" "$role" "$input" > "$f"
  log="$FWF_STATE_DIR/parked-flag/log"
  mkdir -p "$(dirname "$log")" 2>/dev/null
  printf '%s role=%s input=%s reason=%s\n' "$(date -u +%FT%TZ)" "$role" "$input" "$reason" >> "$log"
}
