...
- **A real lint finding that three releases never reached** (code c9f261f,
  ...
  our own runners surfaced it on the first attempt that was allowed to finish.
  Fixed the way #458 fixed the same shape: the capture is consumed by a real
  assertion — a guard that refuses a body must not echo that body to stdout —
  rather than deleted or suppressed.

### Notes

- The `Lint` step takes ~11 minutes on `local-devbox` and `shellcheck` on
  `test/run.sh` alone was observed being SIGKILLed at the memory cliff #480/#520
  describe. The fused single-invocation lint remains the weak point.
...
### Added
- **`fwf upgrade`: the dash banners running-vs-installed binary drift** (#153, code 7e61230, docs 7e61230) — a long-lived dash process could keep rendering from a binary that no longer matched the installed one, so the board quietly described a version of the factory that was no longer running. The dash now detects the drift and banners it instead of presenting stale state as current.
...
### Fixed
- **`fwf authz`: the sentinel match is anchored to column 0** (#218, code b55a90b, docs b55a90b) — the authorization oracle matched its sentinel anywhere in a comment, so quoted or indented text could mint a false `AUTHORIZED` verdict. The match is now anchored per the documented convention, with fixtures covering fenced, indented, quoted-refusal and wrong-issue shapes. This is the oracle the operator un-gates from; a forgeable verdict there is the whole ballgame.
- **`fwf gate` no longer discards a valid verdict on any tip move** (#254, code ab1577d, docs ab1577d) — the ruling is **ancestry, not movement**: a verdict stayed valid if the new tip still descends from the gated SHA, but the gate threw it away whenever the tip changed at all, wedging promotion until the tip moved again (a catch-22 that cost real time on earlier cuts). Ancestry is now the test.
- **Memory-admission control reverted to opt-in (OFF by default)** (#286, code 730eb2f, docs 39f8262) — v0.32.1 flipped `FWF_MEM_ADMIT_ENABLE`'s default to `1` without the real-box calibration criterion (3) of #156 requires. The inequality admission enforces (`free - reserved >= reserve + FWF_MEM_ADMIT_FLOOR_GB`) needs **>= 14 GiB free for the first `--cargo-build` holder and >= 20 GiB for the second** at the shipped floor/reserve placeholders — so on any smaller box it is not slow and not flaky but **unsatisfiable**: every gate waited the full 900s `FWF_MEM_ADMIT_TIMEOUT` and exited `EX_SKIPPED`. **This blocked every release from 2026-08-25 until now** (v0.32.1 and the first v0.33.0 cut were both tagged but never published), and quietly broke any developer machine under ~14 GiB. Reverted to `0`, restoring the configuration measured green on both CI platforms at v0.32.0. The mechanism is unchanged and stays in the tree for a calibrated, canaried re-enable — see `docs/proposals/156-build-serialization.md`. Also closes the #211 gap this exposed: `fwf_free_ram_gb` now reports `UNKNOWN` rather than fabricating a `0` when its probe cannot be read (admission still refuses on `UNKNOWN`, but no longer claims a measurement it never took).
...
- **Build-provenance stamp on every factory PR** (#104, code 12cf8f9, docs 12cf8f9) —
  ...
  [docs/proposals/80-build-provenance-stamp.md](proposals/80-build-provenance-stamp.md).
...
- **Consulting factory template** (code 3cc276b, docs 3cc276b) — a 7th shipped
  ...
  scaffolds under `eval/`. Adds [docs/consulting-factory.md](consulting-factory.md)
...
- **`user-testing` factory template** (#42). A versioned template whose instance
  ...
  - **Runbook**: [`docs/user-testing.md`](user-testing.md) — the full
...
### Added
- **Comprehensive docs**: a hands-on **[tutorial](tutorial.md)** covering
