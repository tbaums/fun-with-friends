...
A git-clone install fetches tags and ff-merges to the latest release tag
(refusing if you have local edits), so it converges on the same commit a
tarball install gets; a tarball install downloads the latest release *next
to* the current directory and re-points the `fwf` symlink, leaving the old
directory in place for rollback. A git **worktree** install (every fwf-self
swarm role runs from one) never merges itself in place — that's unsafe on a
feature branch, detached HEAD, or dirty tree — and instead refuses with the
exact command to run from its main checkout.
