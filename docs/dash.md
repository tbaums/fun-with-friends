...
against the manifest, and only then published (`--draft=false`) — a missing
or unexpected asset fails the workflow with nothing ever made public, rather
than shipping an incomplete release silently (the v0.27.4 incident this
closes).
...
| roles | tmux pane liveness (`@l` label + current command) first, then a chain of positive/negative signals when no pane is found: **live** / **idle** (pane present — always wins, regardless of any other signal) → **floor_idle** (deliberately parked by `fwf-down.sh --floor-only`/`--build-only`) → **unknown** (this role's tmux session itself isn't visible from this host/socket — never rendered as down) → **busy** (no pane, but the role holds its own gate lock — issue #193 AC i) → **stale** (session visible, a heartbeat file exists but is aging) → **down** (session visible, no pane, no lock, no heartbeat trace at all). See issue #193. **The roster itself is the union of the configured `FWF_PAIRS` roles and every name with a file in the heartbeat directory** (issue #402) — so a role that ticked outside the profile's own `FWF_PAIRS` (e.g. after a session-scoped `fwf scale`) still shows, in its real state, instead of silently vanishing from the tab. `FWF_PAIRS` keeps its other job — what `fwf up`/`fwf scale` *provision* — unchanged; only what the dash *displays* changed. **The heartbeat/tick freshness this reads is no longer only a byproduct of a role's own loop restarting** (issue #426): `fwf-gate.sh` — the one shared entrypoint every role's gate/e2e run already goes through — bumps it on every gate attempt, including one it decides to SKIP, so a role deep in several back-to-back gate cycles stays fresh instead of reading `stale` for hours between full loop restarts. Never read the raw `state/<profile>/tick/<role>` file's mtime directly as a liveness verdict on its own — it is exactly one input in the chain above, and the file's own contents are a monotonic counter, not a timestamp. |
