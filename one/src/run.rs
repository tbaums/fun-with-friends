...
pub fn reviewed_issues(events: &[crate::log::Event]) -> std::collections::BTreeSet<u64> {
    use crate::log::Kind;
    use crate::types::IssueState;
    events
        .iter()
...
pub fn fast_track_noted(events: &[crate::log::Event]) -> std::collections::BTreeSet<u64> {
    use crate::log::Kind;
    events
        .iter()
        .filter_map(|e| match &e.kind {
...
pub fn run(cfg: &RunConfig, apps: &Apps) -> Result<(), String> {
    ...
    loop {
        ...
        // Review is the gate now, and the record is where it is written
        // (#630): an issue reaches an impl seat because a human signed it
        // off, or because someone applied `fast-track` by hand.
