...
mod tests {
    ...
    /// The claim ref this floor left behind is readable, so a cycle can tell
    /// its own claim from someone else's instead of failing hard (#602).
    #[test]
    fn the_claim_ref_upstream_can_be_read_back() {
        let (root, up, _work, base) = upstream();
        let url = format!("file://{}", up.display());
        let m = Mirror::init(&root, &url).unwrap();
        assert_eq!(m.upstream_claim_ref(602, "").unwrap(), None);
        let fence = m.create_claim_ref(602, &base, "").unwrap();
        assert_eq!(
            m.upstream_claim_ref(602, "").unwrap(),
            Some(Sha::parse(&fence.0).unwrap())
        );
        assert!(matches!(
            m.create_claim_ref(602, &base, "").unwrap_err(),
            MirrorError::ClaimTaken(602)
        ));
        m.release_claim_ref(602, &fence, "").unwrap();
        assert_eq!(m.upstream_claim_ref(602, "").unwrap(), None);
        let _ = std::fs::remove_dir_all(up.parent().unwrap().join("nothing"));
        let _ = std::fs::remove_dir_all(root);
    }
